...
namespace sbx.repositories.Ciudad
{
    public class CiudadRepository: ICiudad
    {
        ...
        public async Task<Response<dynamic>> ListCiudadForDepartament(int IdDepartamento)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                var response = new Response<dynamic>();
                ...
                try
                {
                    ...
                    string sql = "SELECT IdCity, Code, CityName, IdDepartment FROM T_City WHERE IdDepartment = " + IdDepartamento;
